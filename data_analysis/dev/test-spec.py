...
#WEIGHT = np.array([1, 1, 1, 1, 1, 1, 1, 1/2, 1/2, 1])## different for get doppler

# LOS for the ROI -- these are OLD LOS positions from circa 2024 September to 2025 March
LOS_2409 = (np.array([
        6, 5, 8, 3, 10,
        1,
        11, 2, 9, # 9 is commented in get_doppler_shift
        4, 7
]) - 5.8) * 15
...
def gaussian(x, area, center, sigma, offset):
    ...
    """
    return (area / (sigma * np.sqrt(2 * np.pi))) * np.exp(-0.5 * ((x - center) / sigma)**2) + offset

def twogauss(
    x,              # input wavelength axis
    total_area,     # total area under both Gaussians
    center,         # central peak location
    physical_width, # physical broadening (e.g. Doppler)
    ...
    component1 = gaussian(
        x,
        area=(1 - alpha) * total_area,
        center=center + delta0,
        sigma=np.sqrt(physical_width**2 + sigma0**2),
        offset=0
    )
    ...
    component2 = gaussian(
        x,
        area=alpha * total_area,
        center=center + delta1,
        sigma=np.sqrt(physical_width**2 + sigma1**2),
        offset=0
    )

    return component1 + component2 + baseline
    ...
    def fit_CIII(self,
        ...
        for i,roi in enumerate(self.roi):
            ...
            try:
                ...
                print(f"Fit failed for ROI {roi_val}: {e}")
                popt = np.full(8, np.nan)
            ...
            Ti = joule_to_eV((width * cvel / lam0)**2 * mC)
